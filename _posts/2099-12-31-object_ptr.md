...
There are already three smart pointers in `std`: `unique_ptr`, `shared_ptr` and `weak_ptr`.
...
The idea behind `observer_ptr` is to provide an interface that indicates the pointer that is being passed is considered read-only and there will be no intention of extending the underlying object lifetime or stealing its ownership. If you use `std::shared_ptr<Something> const&` in the function parameter list, you can use `observer_ptr<Something>` instead.

Why not simply use `Something*` instead then? You *could* but you probably shouldn't. Raw pointers inject many quirks into the code -- can be incremented and decremented, compared using `<`, converted into `void*`, casted. Additionally, open up new questions, most importantly: *"who owns the object?"*.
...
On top of that, it requires a better name. `observer` implies relation to the Observer pattern, while, if fact, has nothing to do with it.
...
Most of the time, it does not matter whether the `std::shared_ptr<T>` or `T*`, or `boost::shared_ptr<T>`, or `my::better_ptr<T>` is being provided by the caller. If all you require is read access to the underlying object, dependency on the specific type can be removed.
Not only the function is going to end up being more generic but also a lot more readable.

For `something(std::unique_ptr<T> const&, ...)` --> `something(ptr_view<T>, ...)` gets rid of the question *can this object be used in multiple invocations?*.

For `something_else(T* const, ...)` --> `something_else(ptr_view<T>, ...)` any ownership-related concerns are removed from the scope.
...
`ptr_view` behaves also a bit like `std::weak_ptr`'s *dumber* friend - but in a good way. Often you can be bound to use `std::weak_ptr<T>` as a mitigation for possible ownership cycles. However, assuming one of the objects is guaranteed to outlive the other one, it would be sufficient to store non-owning pointer to the longer-living one, like `T*`. On, you guessed it, `ptr_view<T>`.
...
Of course, we can get back to the discussion that we can just use `Executor*` but it will be as less-safe, less-readable and less-explicit compared to vocabulary type like `ptr_view`, or `object_ptr`, or other name that will be finally chosen.


## `valid_ptr<T>`
One possible extension of the non-owning pointer could be a `valid_ptr` concept, that is never pointing to some existing object, and never to the `nullptr`.
...
That immediately leads to a discussion what should be done in case `ptr == nullptr`. `assert_valid()` can throw exception on invalid ptr or truly `assert` that condition and terminate on fail. That would be a question for a long and, probably, unsettlementable discussion.
...
I hope we get it merged into the standard soon.
